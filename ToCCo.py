...
def conjv(vect):
    ve = matrix_to_vector(((vect).to_matrix(C).H).xreplace({conjugate(x):x,conjugate(y):y,conjugate(z):z,conjugate(t):t,conjugate(et):et,conjugate(ev):ev}),C)
    return(ve)

def conj(scal):
    ve = conjugate(scal).xreplace({conjugate(x):x,conjugate(y):y,conjugate(z):z,conjugate(t):t,conjugate(et):et,conjugate(ev):ev})
    return(ve)
...
def null_space(A):
    ...
    for inc in reversed(range(8)):
        if mp.almosteq(s[inc], 0, (10**(-(mp.mp.dps / 3))) ) == True:
            ...
            divss = N(simplify((divergence(Btest * ansatz.xreplace({kz: solrnow}))) / ansatz.xreplace({kz: solrnow})))
...
def Bound_nosolve(U, B, psi, psi_2b, dic, order_v,order_t):
    ...
    if (condB == "harm pot"):
        ...
        if params_dexp.Bound_nb == 2:
            ...
            if  order_v == 'no':
                TEq = [Eq_n1, Eq_n2,Eq_bx, Eq_by, Eq_bz, Eq_b2x, Eq_b2y, Eq_b2z]
            else:
    ...
    if (condB == "Thick"):
        ...
        if params_dexp.Bound_nb == 2:
            ...
            conservation_E_2 = nn2.cross(qRm*curl(B)-qRmc*curl(psi_2b)-(U.cross(B)))
            ...
            if  order_v == 'no':
                TEq = [Eq_n1, Eq_n2,Eq_bx, Eq_by, Eq_bz, Eq_b2x, Eq_b2y, Eq_b2z, Eq_Ex,Eq_Ey, Eq_E2x,Eq_E2y]
            else:
                TEq = [(taylor(eq, order_v,order_t, dic)) for eq in [Eq_n1, Eq_n2,
...
if order== 2:
    orders = [(0, 0), (0, 1), (1, 0), (1, 1),(0, 2),(2, 0), (1, 2), (2, 1), (2, 2)]
...
for i in range(0, len(orders)):
    ...
    if iv == 0 and it==0:
        ...
        if params_dexp.condB=='Thick':
            ...
            psi = Symbol('psi0x')*C.i+Symbol('psi0y')*C.j+Symbol('psi0z')*C.k
            psi=psi+(Symbol('psi0x_z')*C.i+Symbol('psi0y_z')*C.j+Symbol('psi0z_z')*C.k)*z

            if Bound_nb == 1:
                symb = [Symbol('psi0x'),Symbol('psi0y'),Symbol('psi0z'),Symbol('psi0x_z'),Symbol('psi0y_z'),Symbol('psi0z_z')]
            if Bound_nb == 2:
        ...
        p= P0.xreplace(solve(eqP,[Symbol('P0_x'),Symbol('P0_y'),Symbol('P0_z')])).xreplace({**dico0,**dico1})
        ...
        if params_dexp.condB =="Thick":
            ...
            for ex in expo:
                ...
                B0 += (Symbol('BOx')*C.i+Symbol('BOy')*C.j+ Symbol('BOz')*C.k)*ev*ansatz
                psi0 += (Symbol('bmx')*C.i+ Symbol('bmy')*C.j + Symbol('bmz')*C.k)*ev*ansatz
            ...
            for ext in exp_time:
                ...
                sokzpsi = sokzpsi[[mp.im(m.xreplace({**dico0, **dico1}).evalf(mp.mp.dps)) > 10**(-mp.mp.dps) for m in sokzpsi]][0].xreplace({**dico0, **dico1}).evalf(mp.mp.dps)
                psi1 = psi1 +(Symbol('bmx')*C.i+ Symbol('bmy')*C.j)*ext.xreplace({kz:sokzpsi})*ev
                ...
                sokzb = sokzb[[mp.im(m.xreplace({**dico0, **dico1}).evalf(mp.mp.dps)) < -10**(-mp.mp.dps) for m in sokzb]][0].xreplace({**dico0, **dico1}).evalf(mp.mp.dps)
                B1 = B1 +(Symbol('BOx')*C.i+Symbol('BOy')*C.j)*ext.xreplace({kz:sokzb})*ev


            psi_2b = 1

            Boun0 = Bound_nosolve(U, B1,psi1 ,psi_2b,{**dico0, **dico1}, 1,0)
            ...
            for ext in exp_time:
                ...
                soB0 = solve(Boun0.applyfunc(coe),dict=True,rational=False)[0]
        ...
        for ansatz in expo:
            ...
            tilt = (simplify((gradient(ansatz0)/ansatz0)).normalize()).to_matrix(C)
            if sum(abs(tilt)) == 0:
            ...
            Mp1, rmep1 = linear_eq_to_matrix(eqp.xreplace(dict(zip(var_keep, varom))).doit(), varom)
            ...
            if pa == Matrix(0, 1, []) :
                ...
                Aeq_k =Aeq
                Beq_k =Beq
                eqp = (A1 * Matrix(var_keep)).diff(t) - Aeq * Matrix(var_keep) - Beq
                #
                # ######
                ...
                var_t = Matrix(np.array(var_keep)[list(np.sum(A1,axis =0) == 1)])
                ...
                try:
                    ...
                    print('Gauss pivot failed')
                    print('test pinv')
                    part = Aeq.pinv_solve(-Beq,arbitrary_matrix = Matrix([0,0,0,0,0,0]))
    ...
    if i > 1 :
        ...
        if params_dexp.test == 1:
            ...
            testequations = simplify(testequations.xreplace({x:1,y:0,z:-1,t:0})).evalf()
            testequations = simplify(testequations.evalf())
            print('Residuals = ')
            print(testequations)
    #     #
        Ubnd = U + ev**iv * et**it * Usopart #+ ev**iv * et**it * (Function('Uhomx')(x, y, z, t)
                                            #* C.i + Function('Uhomy')(x, y, z, t) * C.j + Function('Uhomz')(x, y, z, t) * C.k)
        Bbnd = B + ev**iv * et**it * Bsopart #+ ev**iv * et**it * (Function('Bhomx')(x, y, z, t)
                                            #* C.i + Function('Bhomy')(x, y, z, t) * C.j + Function('Bhomz')(x, y, z, t) * C.k)
        Ubnd = U + ev**iv * et**it * Usopart #+ ev**iv * et**it * (Symbol('Uhomx')* C.i + Symbol('Uhomy') * C.j + Symbol('Uhomz') * C.k)
        Bbnd = B + ev**iv * et**it * Bsopart #+ ev**iv * et**it *  (Symbol('Bhomx')* C.i + Symbol('Bhomy') * C.j + Symbol('Bhomz') * C.k)

    # Calculate the boundary conditions equations
        psiBnd = psi
        psiBnd_2 = psi_2b

        TEq = Bound_nosolve(Ubnd, Bbnd, psiBnd, psiBnd_2, {**dico0, **dico1}, iv,it)
        expo = findexp(TEq)
        # if iv == 1 and it == 1:
        ...
        eq = makeEquations(Uh, Bh, ph, rhoh, iv,it, {**dico0,**dico1}) # {**dico0, **dico1})
        var = [Symbol('u' + str(i) + 'x'), Symbol('u' + str(i) + 'y'), Symbol('u' + str(i) + 'z'), Symbol('p' + str(i)),
               Symbol('b' + str(i) + 'x'), Symbol('b' + str(i) + 'y'), Symbol('b' + str(i) + 'z'), Symbol('rho' + str(i))]
        ...
        for j, ansatz in enumerate(expeq):
            ...
            if params_dexp.condB == "Thick":
                ...
                if buf ==1:
                    ...
                    kzpsi[j] = np.array(solve(simplify((diff(psian,t)-qRmm*laplacian(psian)) / psian).xreplace({**dico1,**dico0}), kz,rational=False))
                kzpsi[j] = np.array([kzps.evalf(mp.mp.dps) for kzps in kzpsi[j]])
                kzpsi[j] = kzpsi[j][[
                    np.imag(np.complex128(kps)) >= 0 for kps in kzpsi[j]]][0]
                psiBnd = psiBnd + ev**iv * et**it * \
                    ((Symbol('bmx')*C.i+ Symbol('bmy')*C.j + Symbol('bmz')*C.k) * ansatz).xreplace({kz: kzpsi[j]})

                if Bound_nb == 2:
                    kzpsi_2[j] = np.array(solve(simplify((diff(psian,t)-qRmc*laplacian(psian)) / psian).xreplace({**dico1,**dico0}), kz,rational=False))
                    kzpsi_2[j] = np.array([kzps.evalf(mp.mp.dps) for kzps in kzpsi_2[j]])
                    kzpsi_2[j] = kzpsi_2[j][[
    ...
    # Decompose equations by exponential create and solve matrix
        ...
        if params_dexp.condB == "Thick":
            ...
            if Bound_nb == 2:
                coeffs = [Symbol('C' + str(i)) for i in range(len(Eqbound)-6)]+ [Symbol('bmx_2'),Symbol('bmy_2'),Symbol('bmz_2')]+[Symbol('bmx'),Symbol('bmy'),Symbol('bmz')]
        ...
        for j, ansatz in enumerate(expeq):
            ...
            try:
                ...
                abc = Mat.pinv_solve(res,arbitrary_matrix = Matrix([0,0,0,0,0,0,0,0]))
            ...
            for l in range(len(sol[j])):
                ...
                solhom = solhom + \
                    abc[l] * Matrix(eig[j][l]) * ansatz.xreplace({kz: sol[j][l]})
    ...
    # Expression of the final calculated variable Un
        ...
        U = (U.xreplace({**dico0, **dico1}).xreplace({**dico0, **dico1})).xreplace({Symbol('tau0'):0})#.xreplace({Symbol('tau1'):1})
        B = (B.xreplace({**dico0, **dico1}).xreplace({**dico0, **dico1})).xreplace({Symbol('tau0'):0})#.xreplace({Symbol('tau1'):1})
        rho = (rho.xreplace({**dico0, **dico1}).xreplace({**dico0, **dico1})).xreplace({Symbol('tau0'):0})#.xreplace({Symbol('tau0'):1}).xreplace({Symbol('tau1'):1})
        p = (p.xreplace({**dico0, **dico1}).xreplace({**dico0, **dico1})).xreplace({Symbol('tau0'):0})#.xreplace({Symbol('tau0'):1}).xreplace({Symbol('tau1'):1})
    ...
    if params_dexp.test == 1:
        ...
        testbound = mp.chop(mpmathM(testbound.evalf(3)),tol = 10**(-mp.mp.dps/1.2))
        ...
        testequations = mp.chop(mpmathM(testequations.evalf(3)),tol = 10**(-mp.mp.dps/1.2))
...
if params_dexp.write_file == True:
    ...
    if Bound_nb == 1:
        ...
        files.write(str({**dico1, **{"Bound": Bound_nb, 'f1': topo1,'meta':dic_meta}}))
